...
Да, помню. **StoreForge** — учебный интернет-магазин на микросервисах: FastAPI, PostgreSQL, Redis, RabbitMQ, OpenTelemetry и другие инфраструктурные компоненты.

`Auth Service` должен отвечать только за **идентификацию пользователя, вход в систему и выдачу прав доступа**. За товары, корзину и заказы он отвечать не должен.
